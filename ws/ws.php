...
function put_galeria($token , $data ) {
      ...
      $path_guardar="/var/www/admin/webservices/admin/youtterUploads";
...
function verificar_token($bd, $token) {
    ...
    if ( COUNT($res) >= 1 ) {
		...
		$q2="SELECT * FROM Usuarios WHERE id = '".$r['id_usuario']."' ;";
		//echo $q."<bR>";
		$res2 = $bd->consultar($q2);

		if ( COUNT($res2) >= 1 ) {
			 $r['id_cliente']= $res2[0]['id_cliente'];
		}
